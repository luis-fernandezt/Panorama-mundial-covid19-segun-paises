...
#load data
full_data <- read_excel("./total_cases_per_million.xlsx")
...
ggplot(full_data, aes(x=elapsed_days, y=total_cases_per_million, group=location, color=location)) +
  ...
  geom_text(aes(label = iso_code), size= 4, hjust = -0.1, data = pais_compare_cases %>% filter(date == max(date))) +
...
full_data_2 <- read_excel("./total_deaths_per_million.xlsx")
summary(full_data_2)
...
ggplot(data=full_data_2, aes(x=elapsed_days, y=total_deaths_per_million, group=location, color=location)) +
  ...
  geom_text(aes(label = iso_code), size= 4,hjust = -0.1, data = pais_compare_deaths %>% filter(date == max(date))) +
  scale_colour_viridis_d(option = 'D', begin = 0,end = 0.8, direction = -1,alpha=0.8) +
